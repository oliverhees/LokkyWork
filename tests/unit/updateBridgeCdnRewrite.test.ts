...
const makeGitHubReleaseResponse = () => [
  {
    ...
    name: 'v1.9.22',
    body: 'release notes',
    html_url: 'https://github.com/oliverhees/LokkyWork/releases/tag/v1.9.22',
    published_at: '2026-04-29T00:00:00Z',
    prerelease: false,
    draft: false,
    assets: [
      {
        name: 'LokkyWork-1.9.22-mac-arm64.dmg',
        browser_download_url:
          'https://github.com/oliverhees/LokkyWork/releases/download/v1.9.22/LokkyWork-1.9.22-mac-arm64.dmg',
        size: 123,
        content_type: 'application/x-apple-diskimage',
      },
      {
        name: 'LokkyWork-1.9.22-win-x64.exe',
        browser_download_url:
          'https://github.com/oliverhees/LokkyWork/releases/download/v1.9.22/LokkyWork-1.9.22-win-x64.exe',
        size: 456,
        content_type: 'application/vnd.microsoft.portable-executable',
      },
      {
        name: 'LokkyWork-1.9.22-linux-amd64.deb',
        browser_download_url:
          'https://github.com/oliverhees/LokkyWork/releases/download/v1.9.22/LokkyWork-1.9.22-linux-amd64.deb',
        size: 789,
      },
    ],
  },
...
describe('updateBridge asset URL mapping', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('serves asset.url straight from the GitHub release (no CDN mirror)', async () => {
    ...
    try {
      ...
      const result = await handler({ repo: 'oliverhees/LokkyWork' });
      ...
      const macAsset = assets.find((a: { name: string }) => a.name === 'LokkyWork-1.9.22-mac-arm64.dmg');
      expect(macAsset).toBeDefined();
      // url and fallbackUrl both point at the GitHub release asset directly.
      expect(macAsset?.url).toBe(
        'https://github.com/oliverhees/LokkyWork/releases/download/v1.9.22/LokkyWork-1.9.22-mac-arm64.dmg'
      );
      expect(macAsset?.fallbackUrl).toBe(macAsset?.url);
      expect(macAsset?.url).not.toMatch(/static\.aionui\.com/);

      const linuxAsset = assets.find((a: { name: string }) => a.name === 'LokkyWork-1.9.22-linux-amd64.deb');
  ...
  it('accepts github.com release URLs for download', async () => {
    ...
    try {
      ...
      const result = await handler({
        ...
        url: 'https://github.com/oliverhees/LokkyWork/releases/download/v1.9.22/LokkyWork-1.9.22-mac-arm64.dmg',
        file_name: 'LokkyWork-1.9.22-mac-arm64.dmg',
      });
