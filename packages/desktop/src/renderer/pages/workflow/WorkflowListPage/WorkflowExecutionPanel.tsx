  ...
  return (
    <div className='mt-12px rounded-12px border border-solid border-[var(--color-border-2)] bg-fill-2 px-14px py-12px'>
      <div className='mb-10px flex items-center gap-8px'>
        ...
        <Tag size='small' color={execution.status === 'error' ? 'red' : execution.status === 'completed' ? 'green' : 'arcoblue'}>
          ...
          return (
            <div key={result.step_id} className='flex items-center justify-between gap-10px'>
              ...
              {result.conversation_id ? (
                <Button type='text' size='mini' icon={<Right size='14' />} onClick={() => navigate(`/conversation/${result.conversation_id}`)}>
